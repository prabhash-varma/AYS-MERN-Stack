...
function RequestDetails() {
  ...
  //   e.preventDefault();
  //   const res = await Axios.patch(`http://localhost:3001/orders/${temp.id}`, {
  //     cost: amount,
  //     status: 1,
  //   });
  //   const res1 = await Axios.patch(
  //     `http://localhost:3001/employees/${temp.eid}`,
  //     { free: 1 }
  //   );
 ...
 {item.status === 0 ? (
                 ...
                 <input type="radio" /><h4>
                      ...
                      const res = await Axios.post(`https://ays-backend.azurewebsites.net/updateorder`,{orderid :item._id,cost: amount, status: 1},{headers:{"authorization":`bearer ${localStorage.getItem("token")}`}});
                     const res1 = await Axios.post(`https://ays-backend.azurewebsites.net/updateemployeebyemail`,{email:item.eemail,free:1},{headers:{"authorization":`bearer ${localStorage.getItem("token")}`}})
