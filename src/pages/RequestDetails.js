...
function RequestDetails() {
 ...
 {item.status === 0 ? (
                 ...
                 <input type="radio" /><h4>
                      ...
                      const res = await Axios.post(`http://localhost:3001/updateorder`,{orderid :item._id,cost: amount, status: 1},{headers:{"authorization":`bearer ${localStorage.getItem("token")}`}});
                     const res1 = await Axios.post(`http://localhost:3001/updateemployeebyemail`,{email:item.eemail,free:1},{headers:{"authorization":`bearer ${localStorage.getItem("token")}`}})
